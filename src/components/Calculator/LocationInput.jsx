...
const LocationInput = ({ 
  ...
  const geocodeCityNominatim = async (cityQuery) => {
    try {
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(cityQuery + ' airport')}&limit=8&addressdetails=1`,
        {
          headers: {
      ...
      console.warn('Nominatim API failed, falling back to local database:', error);
      return [];
    }
  };
  ...
  const performSearch = async (query) => {
    ...
    try {
      ...
      if (uniqueResults.length > 0) {
        ...
        setSearchError('No locations found. Try searching for a major city or airport name.');
        setSuggestions([]);
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-4 relative location-search-container">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center mb-3">
          <LocationIcon className="w-5 h-5 text-[#5948DB] mr-2" />
          ...
          <div className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-xl max-h-80 overflow-y-auto">
            {suggestions.map((location, index) => (
              <button
                ...
                className="w-full px-4 py-3 text-left hover:bg-blue-50 border-b border-gray-100 last:border-b-0 focus:outline-none focus:bg-blue-50 transition-colors duration-150"
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-semibold text-gray-900">
                        {location.type === 'airport' ? location.city : location.name}
                      </p>
                    ...
                    <p className="text-sm text-gray-600">
                      {location.type === 'airport' ? location.name : location.fullAddress}
                    </p>
                    {location.country && (
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="flex items-center mb-2">
                ...
                <p className="text-sm font-semibold text-[#5948DB]">Location Selected</p>
              </div>
              <p className="text-sm text-[#5948DB] mb-1">{selectedCity}</p>
              <p className="text-xs text-[#5948DB]">
                <strong>Coordinates:</strong> {parseFloat(coords.lat).toFixed(4)}, {parseFloat(coords.lng).toFixed(4)}
              </p>
            </div>
      ...
      <div className="shadow-md rounded-lg p-3">
        ...
        <div className="grid grid-cols-2 gap-2 mt-2 text-xs text-[#5948DB]">
          <div>• Mumbai (city)</div>
